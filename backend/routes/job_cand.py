...
def job_candidate_helper(job_candidate: dict) -> JobCandidateOut:
    """Convert a raw Mongo job_candidates document into the API response model."""

    return JobCandidateOut(
        jobcand_id=str(job_candidate["jobcand_id"]),
...
async def create_job_candidate(payload: JobCandidateCreate) -> JobCandidateOut:
    """Insert a Job_Candidate relationship document.

    This should be used when the candidate already exists and only the
    candidate-job relationship needs to be created.
    """
    ...
    created_job_candidate = await db.job_candidates.find_one({"jobcand_id": result.inserted_id})
...
async def get_job_candidate(jobcand_id: str) -> JobCandidateOut:
    ...
    job_candidate = await db.job_candidates.find_one({"jobcand_id": ObjectId(jobcand_id)})
