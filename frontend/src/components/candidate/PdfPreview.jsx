...
export default function PdfPreview({ src, label, onDelete }) {
  ...
  return (
    <div className={`${card.flat} overflow-hidden`}>
      <div className="px-4 py-2 border-b border-neutral-100 flex items-center justify-between gap-3">
        <span className="text-sm font-medium text-neutral-500">{label}</span>
        {onDelete && (
          <button
            onClick={onDelete}
            className="rounded-lg border border-coral-200 bg-white px-3 py-1 text-xs font-semibold text-coral-600 transition-colors hover:bg-coral-50"
          >
            Delete {label}
          </button>
        )}
