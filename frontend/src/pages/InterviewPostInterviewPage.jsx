  ...
  return (
    <div className="relative h-24 w-24 shrink-0">
      ...
      <div className={`absolute inset-0 ${flex.colCenter}`}>
        <span className="text-xl font-extrabold leading-none text-neutral-800">{value.toFixed(1)}</span>
        <span className="mt-0.5 text-[9px] font-semibold uppercase tracking-wide text-neutral-400">Overall</span>
      </div>
    </div>
  );
}
...
function ScoreBar({ label, score }) {
  ...
  return (
    <div className={`${flex.col} gap-1`}>
      <div className={`${flex.rowBetween} text-xs`}>
        <span className="text-neutral-600 font-medium">{label}</span>
        <span className="text-neutral-400 tabular-nums">{score.toFixed(1)}</span>
      </div>
  ...
  return (
    <div className={`flex-1 overflow-hidden ${flex.col}`}>
      <div className="flex-1 overflow-hidden flex">
        ...
        {/* Sections sidebar — pushes transcript when open */}
        {hasSections && (
        ...
        <div className={`flex-1 ${flex.col} overflow-hidden`}>
          ...
          {/* Controls — above transcript only */}
          <div className="shrink-0 bg-neutral-50 border-b border-neutral-100 px-6 py-4">
        ...
        </div>{/* end transcript column */}

        {/* Report panel */}
        {candidateReport && (
          <div className="flex-1 border-l border-neutral-200 bg-white overflow-y-auto px-6 py-6">
            <div className={`${flex.col} gap-6`}>

              {/* Scores: donut centered, then each bar with its evidence beneath */}
              {scoreRows.length > 0 && (
                <div className={`${flex.col} gap-4`}>
                  <p className="text-xs font-bold uppercase tracking-wide text-neutral-400">Scores</p>
                            ...
                            {r.evidence.map((ev, i) => (
                              ...
                              >
                                <div className={`${flex.col} gap-0.5 flex-1 min-w-0`}>
                                  ...
                                  <p className="text-xs italic leading-relaxed text-neutral-600 group-hover:text-neutral-800 transition-colors line-clamp-2">
                                    &ldquo;{ev.text}&rdquo;
                                  </p>
                                </div>
                                <svg
              ...
              {/* Full feedback — summary, strengths, improvements, requirements */}
